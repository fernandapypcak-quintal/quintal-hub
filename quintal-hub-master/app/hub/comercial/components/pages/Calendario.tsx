...
export default function Calendario({ filtros }: { filtros: { status: any; unidade: string; ano: string } }) {
  ...
  const { deals, loading } = useDeals({ ...filtros, status: '' }, 1)
  ...
  const monthDeals = deals.filter(d => String(d.data_evento || '').startsWith(yearMonth))
      ...
      <div style={{ background: '#fff', border: '0.5px solid #E8E8E2', borderRadius: 14, padding: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 }}>
          ...
          {Array.from({ length: firstDay }, (_, i) => (
            <div key={`prev-${i}`} style={{ aspectRatio: '1', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#ccc' }}>{prevDays - firstDay + 1 + i}</div>
          ))}
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <div key={d} onClick={() => handleDayClick(dateStr)}
                ...
                {hasEvent && (
                  multiStatus
                    ? <div style={{ display: 'flex', gap: 2, marginTop: 2 }}>{statuses.slice(0,3).map(s => <div key={s} style={{ width: 4, height: 4, borderRadius: '50%', background: isToday ? 'rgba(255,255,255,0.7)' : statusColor(s) }} />)}</div>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #E8E8E2', borderRadius: 14, padding: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontSize: 13, fontWeight: 600 }}>
            {dayEvents ? `Eventos em ${dayLabel} (${dayEvents.length})` : `Eventos de ${MONTHS_SHORT[month]}/${year} (${monthDeals.length})`}
          </span>
          {dayEvents && (
            <button onClick={() => setDayEvents(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: '#9a9c9f' }}>ver mês</button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 400, overflowY: 'auto' }}>
            ...
            {!(dayEvents || monthDeals).length && (
              <div style={{ textAlign: 'center', color: '#9a9c9f', fontSize: 12, padding: 20 }}>Nenhum evento neste período</div>
            )}
          </div>
        )}
      </div>
