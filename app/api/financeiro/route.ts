...
// Proxy server-side para o Apps Script do Dashboard Financeiro (saldos_raw) — evita CORS

import { NextRequest, NextResponse } from 'next/server'
import { getUserAccess, hasDashboardAccess } from '@/lib/permissions'

const GAS_URL = 'https://script.google.com/macros/s/AKfycbyiK7mIxcWaBUU5E4mbq1CtC0qmBoEK082awl35dPJbKH1nCk9JPEeW1-PN9Z7F_EdCRg/exec'
...
export async function GET(req: NextRequest) {
  ...
  if (!hasDashboardAccess(access, 'financeiro')) return NextResponse.json({ erro: 'Acesso negado' }, { status: 403 })
