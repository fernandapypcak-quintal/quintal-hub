...
function ModalComparacaoMeses({ titulo, campoData, mesNum, anoAtual, anoAnterior, filtros, onClose }: {
  ...
  // Impressão numa janela separada — nada do resto da página entra no
  // caminho, então não tem risco de ficar em branco por causa de ancestral
  // escondido ocupando espaço (o problema do truque de CSS anterior).
  function imprimir() {
    const win = window.open('', '_blank')
    if (!win) { alert('O navegador bloqueou a janela de impressão — libera pop-up pra esse site e tenta de novo.'); return }
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(titulo)}</title>
      <style>
        body { font-family: Arial, Helvetica, sans-serif; padding: 28px; color: #222; font-size: 15px; line-height: 1.5; }
        ...
        @media print { .cols { display: flex; flex-direction: column; } }
      </style>
    </head><body>
      <h1>${esc(titulo)}</h1>
