...
import { RelatoriosProvider, useRelatorios } from './hooks/useRelatorios.jsx'
...
function AppInner() {
  ...
  const { loading, error } = useRelatorios()
...
export default function RelatoriosClientApp({ allowedLojas = '*' }: { allowedLojas?: string[] | '*' }) {
  return (
    <RelatoriosProvider allowedLojas={allowedLojas as any}>
      <AppInner />
    </RelatoriosProvider>
  )
}
